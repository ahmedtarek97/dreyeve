...
from train.config import frames_per_seq, h, w, log_dir, callback_batchsize
from train.config import ckp_dir, prd_dir
from train.batch_generators import dreyeve_I_batch, dreyeve_OF_batch, dreyeve_SEG_batch, dreyeve_batch
from train.utils import seg_to_colormap, get_branch_from_experiment_id
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for b in range(0, callback_batchsize):
            ...
            if self.branch == 'image':
                ...
                x_ff_img = np.squeeze(x_ff_img,axis=0)
                #.transpose(1, 2, 0)
                ...
                # fullframe
                i_ff_img = X[0][b]
                i_ff_img = np.squeeze(i_ff_img, axis=0)
                of_ff_img = X[3][b]
                of_ff_img = np.squeeze(of_ff_img, axis=0)
                seg_ff_img = np.squeeze(X[6][b], axis=0)
            ...
            # stitch and write
            #print(x_ff_img.shape)
            #print(z_ff_img.shape)
            #print(y_ff_img.shape)
