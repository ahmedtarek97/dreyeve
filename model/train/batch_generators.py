...
from train.config import dreyeve_dir, frame_size_before_crop, simo_mode, total_frames_each_run, force_sample_steering
from train.config import dreyeve_train_seq, dreyeve_test_seq
from train.config import train_frame_range, val_frame_range, test_frame_range
from train.config import crop_type
from train.utils import palette, read_lines_from_file
...
def load_batch_data(signatures, nb_frames, image_size, batch_type):
    ...
    if batch_type == 'image':
        ...
        B_s = np.zeros(shape=(batchsize,nb_frames, h_s, w_s,3), dtype=np.float32)
        B_c = np.zeros(shape=(batchsize,nb_frames, h_c, w_c,3), dtype=np.float32)
        subdir = 'DREYEVE_DATA'
    ...
    for b in range(batchsize):
        ...
        mean_image = read_image(join(dreyeve_dir,'DREYEVE_DATA','{:02d}'.format(num_run),'mean_frame.png'), channels_first=False, resize_dim=image_size)

        for offset in range(0, nb_frames):

            if batch_type == 'image':

                x = read_image(join(data_dir,'Frames', '{}.jpg'.format(start + offset)),
                               channels_first=False, resize_dim=image_size) - mean_image

                # resize to (256, 256) before cropping
                x_before_crop = resize_tensor(x, new_shape=frame_size_before_crop)
                B_s[b,offset, :, :,:] = resize_tensor(x, new_shape=(h_s, w_s))
                B_c[b,offset, :, :,:] = crop_tensor(x_before_crop, indexes=(hc1, hc2, wc1, wc2))
            elif batch_type == 'optical_flow':
                x = read_image(join(data_dir, '{}.png'.format(start + offset + 1)),
                               channels_first=False, resize_dim=image_size)
                ...
                x = resize_tensor(np.load(join(data_dir, '{}.npz'.format(start + offset)))['arr_0'],
...
def load_saliency_data(signatures, nb_frames, image_size, gt_type='fix'):
    ...
    for b in range(0, batchsize):
        ...
        y_dir = join(dreyeve_dir,"DREYEVE_DATA",'{:02d}'.format(num_run), 'output_frames' if gt_type == 'sal' else 'output_frames')
        # saliency
        y = read_image(join(y_dir, '{}.jpg'.format(start + nb_frames - 1)),
                       channels_first=False, color=False, resize_dim=image_size)


        # resize to (256, 256) before cropping
        y_before_crop = resize_tensor(np.expand_dims(y, axis=2), new_shape=frame_size_before_crop)
...
def load_saliency_data_simo(signatures, nb_frames, image_size):
    ...
    for b in range(0, batchsize):
        ...
        # resize to (256, 256) before cropping
        y_before_crop = resize_tensor(np.expand_dims(y_fix, axis=0), new_shape=frame_size_before_crop)
